function [ c,ceq ] = myConstraint2nonlinear(X)
...
if isTrue(patchStatus_cstr) && ~isTrue(trimStatus_cstr)
    for i = 1: num_cstr_patch
        ...
        if cstrMatrix_patch(i,2) == 1  %position constraints
            ...
            ceq(temp+1) = distance(geops_cstr_patch1(i,:),geops_cstr_patch2(i,:))-x2(i-5)^2;% distance between the points is 2
            temp = temp+1;
        else
            temp = temp;
        end
...
if isTrue(trimStatus_cstr) && ~isTrue(patchStatus_cstr)
    for i = 1: num_cstr_trim
        ...
        if cstrMatrix_trim(i,2) == 1  %position constraints
            ...
            ceq(temp+1) = distance(geops_cstr_trim1(i,:),geops_cstr_trim2(i,:))-x2(i-5)^2;% the first 5 are coincidence constraints
            temp = temp+1;
        else
            temp = temp;
        end
