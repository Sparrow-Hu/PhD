function [ ceq ] = Constraints_analysis2(X)
global     var_ks_g1_u var_ks_g1_v l_p...
    tLoop1 tLoop2  trimStatus_cstr patchStatus_cstr num_Lp1_cstr para_cstr_trim1 para_cstr_trim2   var_ks_g2_u var_ks_g2_v ...
    ...
    var_ws_g2 var_cp_p2 var_ws_p2 var_ks_p2 l_x0 l_x1  surf1 surf2 ;
digits(4); format long
...
j=1;
if l_p>=1
    for i=l_x0+l_x1+1:l_x0+l_x1+l_p
        x2(j) = X(i);
        j=j+1;
    end
end
...
if isTrue(patchStatus_cstr) && ~isTrue(trimStatus_cstr)
    for i = 1: num_cstr_patch
        ...
        if cstrMatrix_patch(i,2) == 1  %position constraints
            ...
            ceq(temp+1) = distance(geops_cstr_patch1(i,:),geops_cstr_patch2(i,:))-x2(i-5)^2;% the first 5 constraints are incidence constraints
            temp = temp+1;
        else
            temp = temp;
        end
...
if isTrue(trimStatus_cstr) && ~isTrue(patchStatus_cstr)
    for i = 1: num_cstr_trim
        ...
        if cstrMatrix_trim(i,2) == 1  %position constraints
            ...
            ceq(temp+1) = distance(geops_cstr_trim1(i,:),geops_cstr_trim2(i,:))-x2(i)^2;%retrieve parameters from x2
            temp = temp+1;
        else
            temp = temp;
        end
        if cstrMatrix_trim(i,3) == 1 %tangent constraints
            pcp1=para_cstr_trim1(i,:);
            [pt1,dpt1] = nrbdeval(srfc_1,nrbderiv(srfc_1),{pcp1(1),pcp1(2)});
            norm1=cross(dpt1{1},dpt1{2});
            pcp2=para_cstr_trim2(i,:);
            [pt2,dpt2] = nrbdeval(srfc_2,nrbderiv(srfc_2),{pcp2(1),pcp2(2)});
            norm2 = cross(dpt2{1},dpt2{2});
            if isym(norm1) | isym(norm2)
                ceq=sym(ceq);
...
if isTrue(patchStatus_cstr) && isTrue(trimStatus_cstr)
    for i = 1: num_cstr_trim%%constraints on trimming curves
        ...
        if cstrMatrix_trim(i,2) == 1  %position constraints
            ...
            ceq(temp+1) = distance(geops_cstr_trim1(i,:),geops_cstr_trim2(i,:))-x2(i)^2;
            temp = temp+1;
        else
            temp = temp;
        end
    ...
    for i = 1: num_cstr_patch%%second,constraints points on patches
        ...
        if cstrMatrix_patch(i,2) == 1  %position constraints
            ...
            ceq(temp+1) = distance(geops_cstr_patch1(i,:),geops_cstr_patch2(i,:))-x2(i)^2;
            temp = temp+1;
        else
            temp = temp;
        end
